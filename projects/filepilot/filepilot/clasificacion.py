...
def clasificar(nombre: str) -> Categoria:
    ...
    el archivo en «otros» (RF-5). Carecen de extensión tanto un nombre sin punto
    como uno que solo lo lleve al principio, como `.gitignore`, o al final
    (RF-6); excluir los ocultos corresponde al recorrido.
    """
    base, punto, extension = nombre.rpartition(".")
    if not punto or not base or not extension:
        return Categoria.SIN_EXTENSION
    return CATEGORIA_POR_EXTENSION.get(extension.lower(), Categoria.OTROS)
