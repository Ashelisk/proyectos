...
**Estado de FilePilot:** [constitución definida](projects/filepilot/docs/constitution.md). CLI local en Python, con Linux, macOS y Windows como plataformas objetivo, sin cuentas ni servicios externos; sin sobrescritura de archivos ni eliminación automática de duplicados. Carpeta: `projects/filepilot/`. La [spec 001 — análisis e informe de una carpeta](projects/filepilot/specs/001-analisis-carpeta/spec.md) y su [plan técnico](projects/filepilot/specs/001-analisis-carpeta/plan.md) están definidos, con las [tareas preparadas](projects/filepilot/specs/001-analisis-carpeta/tasks.md). T1, T2 y T3 están implementadas: paquete instalable, ayuda por comando o módulo, el subcomando `analizar` con sus opciones y sus códigos de uso incorrecto, y la validación de la carpeta indicada, que distingue la ruta inexistente, la que no es un directorio y la que no puede leerse. Veintinueve pruebas superadas y cuatro omitidas por el entorno en Windows con Python 3.14.7. La [validación](projects/filepilot/specs/001-analisis-carpeta/validation.md) recoge la revisión de T1 a T3: T3 queda pendiente de corregir el manejo de un error de resolución de Python 3.11 y de definir el caso de ruta vacía en la spec. T4 es independiente y sigue sin iniciar; todavía no se analiza el contenido de las carpetas ni se ha verificado la compatibilidad con Python 3.11, Linux o macOS.
