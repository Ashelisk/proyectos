...
- RF-19: El modelo principal del programador será Opus 5, con esfuerzo extra (`claude-opus-5`, `xhigh`), indicado explícitamente al iniciar y reanudar sesiones. El coordinador solo ajustará modelo o esfuerzo por exigencia de la tarea o por correcciones fallidas, dejando una justificación local. Tras dos correcciones consecutivas verificadas como fallidas, el siguiente envío elevará el esfuerzo a `max` salvo selección explícita justificada. Se priorizará ajustar esfuerzo antes de cambiar de modelo; no se ampliarán permisos, rondas ni presupuesto. Los fallos de acceso no justifican eludir restricciones ni usar otro modelo silenciosamente.
