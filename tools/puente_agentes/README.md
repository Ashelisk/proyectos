...
- Una salida no cero puede incluir consumo: el JSON se conserva y su estimación se suma antes de validar la entrega, siempre que corresponda a la sesión. `consumo_verificado: false` indica que falta acreditar el último envío, no que fuera gratuito. Un coste inválido, una respuesta ajena o un error nunca habilitan reintentos ni aprobación.
...
- Si el encargo nuevo continúa el mismo trabajo interrumpido, conservar únicamente el presupuesto y los envíos restantes, descontando también los intentos fallidos. Si su consumo es desconocido, aclararlo antes de continuar. Crear otra copia no reinicia los límites autorizados ni elimina restricciones de cuota del proveedor.
